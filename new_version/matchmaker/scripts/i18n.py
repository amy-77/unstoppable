...
21. 每条证据和案例佐证要简短(一句话)
22. 心智模型中必须包含至少1个关于关系推进阶段的博弈模型(不仅是匹配筛选阶段), 覆盖: 对方的心理防御机制识别(回避型封闭/焦虑型试探/理性型审查)、防御化解策略(坦诚降防/情绪共鸣/价值证明)、推进节奏控制(何时加速确认关系/何时后退给空间/何时制造关键转折点). 该模型需区分匹配阶段博弈(筛选谁)和推进阶段博弈(锁定谁), 并给出从匹配到推进的切换信号.
